...
> 作者：唐小漫 ｜ 开发工具：Codex ｜ 版本：v3.1.1（2026-09-18）
...
| 再次启动 | 已经在运行时，双击「启动.bat」会把面板叫到前面（不会开出第二个程序） |
| 开机自启 | 通过注册表 + 程序目录里的启动器（会等系统就绪再启动、失败重试、并写日志） |
| 开机后的提示 | 启动后会先把面板显示 10 秒（设置里可关），之后按你的贴边/托盘设置收回去 |
